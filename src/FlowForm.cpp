...
FlowForm::FlowForm(const Instance &inst) : RSA(inst), nbVar(0){
    ...
    this->setObjective(instance.getInput().getChosenObj());
...
void FlowForm::setObjective(std::vector<Input::ObjectiveMetric> chosenObjective){
    objectiveSet.resize(chosenObjective.size());
    for (unsigned int i = 0; i < chosenObjective.size(); i++){
        Expression myObjective = this->getObjFunctionFromMetric(chosenObjective[i]);
...
Constraint FlowForm::getMaxUsedSlicePerLinkConstraints(int linkIndex, int d){
    ...
    Constraint constraint(-INFINITY, exp, rhs, constraintName.str());
    return constraint;
}
